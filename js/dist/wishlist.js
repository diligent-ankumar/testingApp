...
function loadWishlistItems() {
    ...
    if (!wishlistContainer || !itemCountElement)
        return;
    ...
    wishlistItems.forEach((item) => {
      ...
      <div class="wishlist-item" data-id="${item.id}">
        ...
        <div class="item-details">
          ...
          <p><strong>Price:</strong> $${(_a = item === null || item === void 0 ? void 0 : item.price) === null || _a === void 0 ? void 0 : _a.toFixed(2)}</p>
...
function moveToCart(itemId) {
    ...
    const itemIndex = wishlistItems.findIndex((item) => item.id === itemId.toString());
    if (itemIndex > -1) {
        ...
        if (existingCartItem) {
            ...
            cartItems.push(Object.assign(Object.assign({}, item), { quantity: 1 }));
...
function removeFromWishlist(itemId) {
    ...
    const wishlistKey = `wishlist_${currentUser.email}`;
    let wishlistItems = JSON.parse(localStorage.getItem(wishlistKey) || '[]');
    wishlistItems = wishlistItems.filter((item) => item.id !== itemId.toString());
    localStorage.setItem(wishlistKey, JSON.stringify(wishlistItems));
    loadWishlistItems();
    window.location.reload();
}
